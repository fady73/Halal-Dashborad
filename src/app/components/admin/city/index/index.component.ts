...
import { CityViewModel } from '../city.model';
...
export class IndexComponent implements OnInit {
  ...
  currentPage: PageEnum = PageEnum.City;
  items: CityViewModel[] = [];
  actions:RoleAction[]=[];
  selectedItem: CityViewModel = new CityViewModel();
  modalRef: BsModalRef;
  ...
  ngOnInit() {
    ...
    this._roleService.getPageActions(PageEnum.City).subscribe(res=>{
  ...
  registerHotKey() {
    ...
    if ( this.canAdd() ) {
      this.crud.hotkeys.add( new Hotkey( 'shift+n', ( event: KeyboardEvent ): boolean => {
        this.crud.router.navigateByUrl( "/admin/country/create" );
  ...
  search() {
    ...
    this.items = [];
    this.cityService.get( this.page.orderBy, this.page.isAscending, this.page.options.currentPage , environment.pageSize).subscribe( response => {
      this.page.isSearching = false;
      if ( response.Success ) {
        this.page.isAllSelected = false;
        this.page.options.totalItems = response.Data.Records;
        this.page.options.totalPages = response.Data.Pages;
        this.items = response.Data.Result as CityViewModel[];
  ...
  remove() {
    ...
    this.cityService.remove( this.selectedItem.ID ).subscribe( response => {
  ...
  removeAll() {
    ...
    if ( IDs && IDs.length > 0 ) {
      this.cityService.removeAll( IDs ).subscribe( response => {
        if ( response.Success ) {
  ...
  changeActivationStatusForAll( isActive: boolean ) {
    ...
    if ( activateViewModelList.length == 0 )
      return;
    this.cityService.changeActivationStatus( activateViewModelList ).subscribe( response => {
      if ( response.Success )
        this.alertService.success( response.Message );
      this.items.forEach( item => {
        if ( item.IsSelected ) {
  ...
  changeActivationStatus( model: CityViewModel ) {
